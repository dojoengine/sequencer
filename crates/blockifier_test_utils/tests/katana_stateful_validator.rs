...
use blockifier::{
    ...
    test_utils::{  contracts::FeatureContractTrait, dict_state_reader::DictStateReader},
    transaction::{
        account_transaction::{AccountTransaction, ExecutionFlags},
        errors::{TransactionFeeError, TransactionPreValidationError},
    },
};
use blockifier_test_utils::{cairo_versions::{CairoVersion, RunnableCairo1}, contracts::FeatureContract};
use starknet_api::{
...
fn test_stateful_validator_skip_all() {
    ...
    let execution_flags =
        ExecutionFlags { charge_fee: false, strict_nonce_check: true, only_query: true, validate: false };
...
fn test_stateful_validator_fail_account_validation() {
    ...
    let execution_flags =
        ExecutionFlags { charge_fee: false, strict_nonce_check: true, only_query: true, validate: true };
...
fn test_stateful_validator_fail_fee_check() {
    ...
    let execution_flags =
        ExecutionFlags { charge_fee: true, strict_nonce_check: true, only_query: true, validate: false };
