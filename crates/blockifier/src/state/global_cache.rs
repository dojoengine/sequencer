...
type ContractClassLRUCache = SizedCache<ClassHash, Arc<ContractClass>>;
...
impl GlobalContractCache {
    ...
    pub fn get(&self, class_hash: &ClassHash) -> Option<Arc<ContractClass>> {
        self.lock().cache_get(class_hash).cloned()
    }

    pub fn set(&self, class_hash: ClassHash, contract_class: Arc<ContractClass>) {
